...
function create_sysimg_from_object_file(input_object::String, sysimage_path::String)
    ...
    run(`$(get_compiler()) $(bitflag()) -shared -L$(julia_libdir) -o $sysimage_path $o_file -ljulia $extra`)
    return nothing
end
...
function create_executable_from_sysimg(;sysimage_path::String,
    ...
    cmd = `$(get_compiler()) -DJULIAC_PROGRAM_LIBNAME=$(repr(sysimage_path)) $(bitflag()) -o $(executable_path) $(wrapper) $(sysimage_path) -O2 $rpath $flags`
    @debug "running $cmd"
    run(cmd)
    return nothing
end
